...
public class StateServiceTests
{
    ...
    public void UpdateState_UpdatesState_WhenStateIsValid()
    {
        ...
        mockStateRepository.Setup(repo => repo.GetByIdAsync(9)).ReturnsAsync(new State { Id = 9, Name = "NewState1" });
    ...
    public void DeleteState_DeletesState_WhenStateExists()
    {
        ...
        mockStateRepository.Setup(repo => repo.GetByIdAsync(9)).ReturnsAsync(new State { Id = 9, Name = "State1" });
    ...
    public void UpdateState_ThrowsException_WhenStateToUpdateDoesNotExist()
    {
        ...
        mockStateRepository.Setup(repo => repo.GetByIdAsync(state.Id)).ReturnsAsync((State)null);

        var stateService = new StateService(mockStateRepository.Object);

        // Act & Assert
        var exception = Assert.Throws<KeyNotFoundException>(() => stateService.UpdateState(state));
        Assert.Equal("State to update not found", exception.Message);
    }
    ...
    public async System.Threading.Tasks.Task GetStateById_ThrowsException_WhenStateDoesNotExist()
    {
        ...
        mockStateRepository.Setup(repo => repo.GetByIdAsync(9)).ReturnsAsync(new State { Id = 999, Name = "State" });

        mockStateRepository.Setup(repo => repo.GetByIdAsync(stateId)).ReturnsAsync((State)null);
